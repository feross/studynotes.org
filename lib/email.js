...
exports.notifyOnException = function (obj) {
  ...
  if (obj.err) {
    ...
    message.text += '\n\n'
    message.text += obj.err.code
    message.text += '\n\n'
    message.text += JSON.stringify(obj.err)
    message.text += '\n\n'
  }
  ...
  if (obj.req) {
    ...
    message.text += obj.req.method + ': ' + config.siteOrigin + obj.req.url
    message.text += '\n\n'
    ...
    if (obj.req.params) {
      ...
      message.text += JSON.stringify(obj.req.params, null, '\t')
    }

    if (obj.req.body) {
      message.text += 'HTTP BODY: '
      message.text += JSON.stringify(obj.req.body, null, '\t')
    }
  }
